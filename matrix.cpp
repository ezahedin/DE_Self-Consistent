...
//=============================================================================//
matrix matrix::coli(unsigned int i)
{
    int c=this->cols;
    int r=this->rows;
    matrix tmp(r,1);
    ...
    for (int j=0; j<r; j++) {
        tmp.array1d[j]=this->array1d[j+i*r];
    }
...
int matrix::min()
{
    ...
    int id_min=0;
    double tmp=this->array1d[0];
    
    for (int i=1; i<r*c; i++) {
        if (this->array1d[i]<tmp) {
            tmp=this->array1d[i];
            id_min=i;
        }
    }
...
//=============================================================================//
bool matrix::operator==(const matrix& t1) {
    
    int r=this->rows;
    int c=this->cols;
    
    for (int k=0; k<r*c; k++) {
        if (this->array1d[k]!=t1.array1d[k]) {
            return false;
        }
    }
    return true;
...
//==============================================================================
matrix vec_transpose(matrix t1) {
 
    int r=t1.rows;
    int c=t1.cols;
    matrix tran(c,r);
    for (int i=0; i<r*c; i++) {
        tran.array1d[i]=t1.array1d[i];
    }
